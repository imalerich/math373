...
\begin{align*}
		B_n(x) &= \Sigma_{k=0}^{n}{n\choose k}f(k/n)x^k(1-x)^{n-k} \\
		&= \Sigma_{k=0}^{n}{n\choose k}\frac{k^2}{n^2}x^k(1-x)^{n-k} \\
		&= (\Sigma_{k=0}^{1}{n\choose k}\frac{k^2}{n^2}x^k(1-x)^{n-k})
			+ (\Sigma_{k=2}^{n}{n-2\choose k-2}x^k(1-x)^{n-k}) \\
		&= (\Sigma_{k=0}^{1}{n\choose k}\frac{k^2}{n^2}x^k(1-x)^{n-k})
			+ x^2(\Sigma_{k=2}^{n}{n-2\choose k-2}x^{k-2}(1-x)^{(n-2)-(k-2)}) \\
		&= (\Sigma_{k=0}^{1}{n\choose k}\frac{k^2}{n^2}x^k(1-x)^{n-k})
			+ x^2(\Sigma_{k=0}^{n-2}{n\choose k}x^{k}(1-x)^{n-k}) \\
		&= (\Sigma_{k=0}^{1}{n\choose k}\frac{k^2}{n^2}x^k(1-x)^{n-k})
			+ x^2 \\
		&= {n\choose 1}\frac{1^2}{n^2}x(1-x)^{n-1}
			+ x^2 \\
		&= \frac{n}{n^2}x(1-x)^{n-1}
			+ x^2 \\
		&= \frac{x}{n}(1-x)^{n-1}
			+ x^2 \\
\end{align*}
...
each of the cubic polynomials which define $S$. $S$ is then given by
$$
S(x) = 
\begin{cases}
...
\textcolor[RGB]{240,240,240}{\rule{\textwidth}{0.5pt}}
\inputminted{octave}{naturalcubicspline.m}
\textcolor[RGB]{240,240,240}{\rule{\textwidth}{0.5pt}}
\inputminted{octave}{clampedcubicspline.m}
